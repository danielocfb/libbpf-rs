...
fn maps(object: &Object) -> impl Iterator<Item = Map> {
    MapIter::new(unsafe { object.as_libbpf_object().as_ref() }).map(|ptr| unsafe { Map::new(ptr) })
}
...
fn gen_skel_contents(_debug: bool, raw_obj_name: &str, obj_file_path: &Path) -> Result<String> {
    ...
    write!(
        ...
        impl {name}SkelBuilder {{
            fn retrieve_maps(obj: std::ptr::NonNull<libbpf_sys::bpf_object>) -> libbpf_rs::Result<std::collections::HashMap<std::string::String, libbpf_rs::OpenMap>> {{
