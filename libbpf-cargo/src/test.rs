...
fn generate_btf(prog_text: &str) -> Result<GenBtf<'static>> {
    let btf = btf_gen::generate_and_load(prog_text, None, [OsString::new(); 0])?;
...
"#;
    ...
    let btf = generate_btf(prog_text).unwrap();
